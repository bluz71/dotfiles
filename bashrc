...
alias g='_g() { if [[ $# == 0 ]]; then git st; else git "$@"; fi }; _g'
...
alias lls='_lls() { ll -r --sort=size "$@" | less; }; _lls'
alias llt='_llt() { ll -r --sort=modified "$@" | less; }; _llt'
...
alias rgm='_rgm() { rg -p "$1" | less; }; _rgm'
alias rm='/bin/rm -i'
...
*.m4b=38;5;222:*.conf=38;5;121:*.md=38;5;224:*.ico=38;5;140"
